...
| Artifact | Contents |
|---|---|
| `data/commit_lineage/gcr_molecular_commits.json` | `schema_version: molecular-commit-lineage@0.2.0` (src: data/commit_lineage/gcr_molecular_commits.json `"schema_version": "molecular-commit-lineage@0.2.0"`); 235 `compensated_commits`, a count the validator pins to the receipt's `commit_count` (src: data/verification_runs/gcr_three_surface_commit_traceability_2026-07-27.json `"commit_count": 235,`); 7 `legacy_detailed_commits` (src: data/commit_lineage/gcr_molecular_commits.json `"legacy_detailed_commits": [`) and 13 `protected_paths` (src: data/commit_lineage/gcr_molecular_commits.json `"protected_paths": [`) — those two numbers are list lengths, not literals in the file, so count the array rather than trusting this row; the source-conversation, materialization-route, plan-package, small-loop and final-repo absolute paths, all of them pointing outside this directory (src: data/commit_lineage/gcr_molecular_commits.json `"final_repo_abs": "<target-repo>/"`); `history_rewrite_policy` (src: data/commit_lineage/gcr_molecular_commits.json `"history_rewrite_policy": "do-not-rewrite-existing-commits; compensate every protected historical commit"`); `coverage_through` (src: data/commit_lineage/gcr_molecular_commits.json `"coverage_through": "1bae35b92f43a548092fa306bc46843ac6abe32b"`); `verification_run_abs` |
| `data/verification_runs/gcr_three_surface_commit_traceability_2026-07-27.json` | `schema_version: molecular-commit-verification-run@0.2.0` (src: data/verification_runs/gcr_three_surface_commit_traceability_2026-07-27.json `"schema_version": "molecular-commit-verification-run@0.2.0"`); `status: pass` (src: data/verification_runs/gcr_three_surface_commit_traceability_2026-07-27.json `"status": "pass",`); `commit_count=235`, `compensated_commit_count=235` (src: data/verification_runs/gcr_three_surface_commit_traceability_2026-07-27.json `"compensated_commit_count": 235,`); `failed_commit_count=0` (src: data/verification_runs/gcr_three_surface_commit_traceability_2026-07-27.json `"failed_commit_count": 0,`); `strict_message_count=20` (src: data/verification_runs/gcr_three_surface_commit_traceability_2026-07-27.json `"strict_message_count": 20,`); 235 `commit_shas`; 2 `proof_commands`, and both of them invoke the TypeScript adapter in the authoring workspace, not the Python validator that lives here (src: data/verification_runs/gcr_three_surface_commit_traceability_2026-07-27.json `"bun run loop_wiki/evolve-unknown-discovery-plan-truth/adapters/typescript/runtime/scripts/validate_molecular_commit_lineage.ts --audit-protected-history",`) |
...
derived from the script's own location (src: scripts/validate_molecular_commit_lineage.py `parser.add_argument("ledger", nargs="?", type=Path, default=default_ledger_path(Path(__file__)))`),
plus four flags declared on the same `argparse` parser (src: scripts/validate_molecular_commit_lineage.py `parser.add_argument("--require-current-history", action="store_true")`).
Those flags select modes of very different strength:

| Invocation | What it actually checks |
|---|---|
| default — **not** run by `git_gate.py`, which deliberately keeps it out of `GATES` (src: scripts/git_gate.py `# scripts/validate_molecular_commit_lineage.py is deliberately NOT gated here.`) | ledger and verification-receipt **structure**, SHA lists and counts; the walk over protected history is skipped outright unless a history flag is passed (src: scripts/validate_molecular_commit_lineage.py `    if not audit_protected_history:`). No Git history is read — `git_gate.py` records the measured gap itself (src: scripts/git_gate.py `schema check that never walks history at all (0.07s vs 19s)`). |
...
(src: .githooks/pre-push `python3 "$ROOT/scripts/git_gate.py"`), and that runner's gate list omits this
...
(src: scripts/validate_molecular_commit_lineage.py `raw_path = payload.get("verification_run_abs")`) and
...
(src: tests/test_skill_asset_governance.py `def test_molecular_commit_lineage_selftest_passes() -> None:`).
It asserts the `--selftest` path only, deliberately not the shipped ledger, because a checkout of this
repository does not contain the commits the ledger describes (src: tests/test_skill_asset_governance.py `    # Asserts the selftest, not the ledger. The ledger describes commits in the authoring`);
the green signal it looks for is the selftest banner (src: scripts/validate_molecular_commit_lineage.py `print("SELFTEST GREEN")`).
