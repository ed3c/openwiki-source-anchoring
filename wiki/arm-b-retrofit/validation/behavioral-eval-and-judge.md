...
(src: scripts/eval_autoresearch_composer.py `dataset must be a JSON list, JSON object with cases, or JSONL records`).
...
(src: scripts/sample_autoresearch_traces.py `Validate local-first autoresearch trace samples.`) and a
...
(src: scripts/sample_autoresearch_traces.py `cloud_judge_enabled=false`). Traces stay on the local filesystem;
