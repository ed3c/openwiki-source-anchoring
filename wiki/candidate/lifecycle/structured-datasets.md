...
`--stdout` (src: scripts/render_lifecycle_openwiki.py `Render OpenWiki lifecycle Markdown from structured lifecycle datasets.`);
the gate compares the two strings directly (src: scripts/check_lifecycle_datasets.py `openwiki != rendered.stdout`), so
that page cannot carry commentary. This page holds the commentary.

## The seven artifacts

| Path | Holds |
|---|---|
| `skill_optimization_registry.json` | the managed skills and their `current_status` (src: data/lifecycle/skill_optimization_registry.json `"current_status": "production-seed-candidate"`) |
| `golden_dataset_versions.json` | dataset versions and their case counts (src: data/lifecycle/golden_dataset_versions.json `"case_count": 4,`) |
| `eval_runs/autoresearch_composer_2026-07-23.json` | the recorded run's `result_summary` (src: data/lifecycle/eval_runs/autoresearch_composer_2026-07-23.json `"result_summary": {`) |
| `promotion_records.json` | promotion decisions and their human-admit binding (src: data/lifecycle/promotion_records.json `"human_admit_binding": {`) |
| `dataset_drift_history.jsonl` | per-version drift rows |
| `trace_privacy_classification.json` | per-dataset cloud policy (src: data/lifecycle/trace_privacy_classification.json `"cloud_allowed": false,`) |
| `../autoresearch_traces/failure_trace_samples.jsonl` | the failure traces the drift rows count |

## What `check_lifecycle_datasets.py` enforces (src: scripts/check_lifecycle_datasets.py `Validate structured lifecycle datasets`)
...
Eight fields, each pinned to an exact value:
...
prepared, and explicitly not signed. Six keys must be present, and every one of them is then value-checked —
two against the promotion record's own fields, four against constants written into the gate:
...
Every entry of the gate's `required_literals` list — sixteen of them, that being the list's length rather
than a number written anywhere — must appear on the generated page, including `4/4`, `3/3`, `delta=1.0`,
`candidate_until_human_admit`, `plan-package-materialization`, `EDGE-066`,
`failure_trace_sample_count` and `route_distribution`; a missing one is reported by name
(src: scripts/check_lifecycle_datasets.py `structured lifecycle openwiki missing literal:`). Because that
page is generated, these are effectively assertions about the **renderer**: if
`render_lifecycle_openwiki.py` stops emitting one of them, the gate fails even though the data is intact.
