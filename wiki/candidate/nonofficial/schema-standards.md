...
(src: scripts/check_wiki_graph_sync.py `schema = read_json("data/wiki_graph/schema.json")`).
It carries two independent version strings
(src: data/wiki_graph/schema.json `"event_schema_version": "wiki-event@0.1.0",`), because the event
...
## What `check_wiki_graph_sync.py` actually checks (src: scripts/check_wiki_graph_sync.py `# Validate Wiki -> Event Log -> GraphRAG/Vector RAG sync artifacts.`)
...
1. **Seven required files exist** — `.github/workflows/wiki_graph_sync.yml`,
   ...
   (src: scripts/check_wiki_graph_sync.py `"openwiki/nonofficial/schema-standards.md",`),
...
| `sample_graph.json` | must contain every name in `schema.required_graph_fields` — `nodes`, `edges`, `chunks`, `retrieval` (src: scripts/check_wiki_graph_sync.py `for field in schema.get("required_graph_fields", []):`) |
...
(src: scripts/check_wiki_graph_sync.py `external_write_enabled") is not False:`), so a missing or
...
| Enforcement | Where |
|---|---|
| the three `required_*` field lists | read at check time by `scripts/check_wiki_graph_sync.py`, never by the projector (src: scripts/check_wiki_graph_sync.py `for field in schema.get("required_event_fields", []):`) |
| `schema_version`, `event_schema_version`, `default_embedding_model` | the only schema keys `scripts/sync_wiki_to_graph.py` reads at projection time (src: scripts/sync_wiki_to_graph.py `embedding = schema["default_embedding_model"]`) |
| everything in the numbered list | `scripts/check_wiki_graph_sync.py` (src: scripts/check_wiki_graph_sync.py `print("PASS: wiki graph sync architecture and artifacts")`) |
