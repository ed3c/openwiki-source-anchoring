...
|---|---|---|---|
| `.githooks/commit-msg` | `scripts/validate_commit_message.py` | `<message-file>`, forwarded unchanged as `"$1"` | none |
| `.githooks/pre-push` | `scripts/git_gate.py` | none — the hook passes no arguments | none; exits 125 if any gate mutated the tree, printing |
...
|---|---|---|---|
| `ENABLE_GRAPH_DB_WRITE` | `sync_wiki_to_graph.py::write_external_graph_if_enabled()` | the string `"true"`, compared with `!=` so anything else returns early | `vars.ENABLE_GRAPH_DB_WRITE` in `wiki_graph_sync.yml` — **matches** |
...
| `ENABLE_LLM_JUDGE` | `eval_autoresearch_composer.py::cloud_judge()` | the string `"1"`, anything else returns a `SKIP` verdict | `autoresearch_eval.yml` passes `vars.ENABLE_CLOUD_EVALS`, i.e. `"true"` because the job condition compares it to `'true'` — **mismatch, the branch never activates** |
...
inside the list as a comment rather than an entry,
...
|---|---|---|
| `check_plan_package_compat.py` | `PROJECT-SSOT.md` / `plan-package.compat.yaml` / `.plan-package.lock.yaml` changed — it parses the manifest and requires those paths | [Plan package compatibility](../governance/plan-package-compatibility.md) |
| `test_plan_package_compat.sh` | same — it is a one-line shell wrapper forwarding its argv | same |
| `check_prompt_trace_assets.py` | `data/prompt_trace/**` changed — that is the tree it reads | [Prompt trace assets](../nonofficial/prompt-trace-assets.md) |
| `sync_wiki_to_graph.py` | a wiki page changed and you want the local projection refreshed; run with no flags it defaults to the in-repo wiki root | [Wiki graph sync](../nonofficial/wiki-graph-sync-architecture.md) |
| `real_driver_ablation.py` | you are about to claim a real behavioral delta — it spawns a real agent command and emits a versioned report | [Ablation and benchmark](../validation/ablation-and-benchmark.md) |
...
`validate_goal_constraints.py` does the same when no paths are given
 — and `GATES` invokes every gate with
no arguments, so inside the chain
...
|---|---|---|---|
| `wiki_graph_sync.yml` → `local-first-wiki-graph-sync` | always, on the path filter — the job carries no `if:` | `contents: read`, `actions: read` | none |
...
| `autoresearch_eval.yml` → `local-first-eval` | always, on the path filter — the job carries no `if:` | default | none |
...
| `data/wiki_graph/event_log.jsonl`, `data/wiki_graph/sample_graph.json` | the projection artifacts; only `schema.json` is in the filter, even though the run rewrites both |
...
| `PROJECT-SSOT.md`, `plan-package.compat.yaml`, `.plan-package.lock.yaml` | the project contract itself; the only thing that reads them is the manual `check_plan_package_compat.py`, and (inferred) no workflow path filter names them — grepping `plan-package` and `PROJECT-SSOT` across `.github/` returns no match |
| `skills/*/status.json` | promotion decisions |
| `tests/**` except `tests/test_autoresearch_eval_suite.py` — the one test file any filter names | the governance suite |
...
| the 22-gate chain, stopping at the first failure | `git_gate.py`, hook and `skill_ci.yml` |
