...
(src: scripts/skill_description_linter.py `Lint skill descriptions for dense route boundaries.`);
`scripts/validate_goal_constraints.py` requires goal/constraint phrasing rather than brittle step lists
(src: scripts/validate_goal_constraints.py `use goal/constraints instead of brittle steps`).
...
(src: skills/gemini_interactions/skills.md `load references/deploy_guide.md only for deployment details`),
`references/state_graph.md` for domain-term recovery
(src: skills/autoresearch_composer/skills.md `recover missing Domain terms via references/state_graph.md`).
`scripts/validate_progressive_disclosure.py` enforces the boundary, failing any root skill that leaks
deployment detail (src: scripts/validate_progressive_disclosure.py `deployment detail leaked into root skill`).
The point is context economy: everything in `skills.md` is paid for on every routing decision.

## `cases.json` — the enforced baseline (src: scripts/validator.py `Validate skill cases.json files with the GCR minimum baseline.`)
...
passing by matching anything (src: scripts/validator.py `has weak expected check`), which is how a
...
(src: scripts/synthetic_case_quality_report.py `"max_template_similarity_ratio": max_template_similarity(prompts),`); see
...
(src: scripts/validate_skills_baseline.py `["git", "status", "--porcelain"],`), falling back to all of them
...
## `status.json` — the promotion decision (src: skills/gemini_interactions/status.json `skill-promotion-status@0.1.0`)
...
4. Measure a delta: `python3 scripts/ablation_engine.py --cases skills/<slug>/cases.json`. The
   ...
   (src: scripts/ablation_engine.py `if skill_slug == "autoresearch_composer":`), so a new slug needs a
...
5. A real behavioral claim requires `scripts/real_driver_ablation.py`, which drives an actual agent
   ...
   (src: scripts/real_driver_ablation.py `Run a real agent command against the same cases with and without skill context.`). See
