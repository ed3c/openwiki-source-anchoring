...
| `plan-package.compat.yaml` | the same contract as machine-readable key/values (src: plan-package.compat.yaml `compat_version: plan-package-compat@0.1.0`) |
| `.plan-package.lock.yaml` | the provenance lock |
| `scripts/check_plan_package_compat.py` | the gate (src: scripts/check_plan_package_compat.py `Check final repo compatibility with the generated plan package.`) |
...
(src: scripts/check_plan_package_compat.py `FAIL: final repo contains small-loop path`) — and
`scripts/check_openwiki.py` independently
(src: scripts/check_openwiki.py `forbidden final repo path exists:`) — fail if any of `small-loop`,
...
(src: plan-package.compat.yaml `final_repo_has_small_loop: false`).
The stronger wording `final_repo_small_loop_policy: forbidden` is **not** in the manifest at all: it is
in the provenance lock (src: .plan-package.lock.yaml `final_repo_small_loop_policy: forbidden`), and the
openwiki gate pins it as a required literal of `openwiki/nonofficial/openwiki.yaml` — note the
`nonofficial/` level (src: scripts/check_openwiki.py `"openwiki/nonofficial/openwiki.yaml": [`).
...
`plan_package_input_count: 39` (src: plan-package.compat.yaml `shadow_defense_count: 8`) — though the
gate re-checks only four of them, leaving `governance_template_count` and `shadow_defense_count`
declared but unenforced (src: scripts/check_plan_package_compat.py `manifest.get("lineage_edge_count") != "92"`).
`frozen_source_sha256` binds the frozen source conversation, and the provenance lock carries the same
digest (src: .plan-package.lock.yaml `frozen_source_sha256: 8fe152eb94179a56b6bd1b43a40c50690741ddf2ebc3a95c6b7b5e05776372a4`).
...
> (src: PROJECT-SSOT.md `only through inputs/plan-package-inputs.yaml plus explicit source-ingest packets`)
