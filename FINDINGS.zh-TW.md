...
53 條更正是本文最弱的一環:由同一批 agent 認定、更正、並回報。其中 22 條交給**只讀源碼的 fresh
agent** 重新裁決。

每一筆紀錄被拆成兩句對立陳述——頁面原本說的、與它被改成的——**所有判決詞剝除、順序依索引奇偶洗
牌**，裁決者不知道哪句是哪句。指令要求:源碼沒說就預設 `CANNOT_DETERMINE`、「兩邊各對一半」是合法
裁決、並且**必須寫出它所否決的那一句還能怎麼辯護**。
...
**抽樣中沒有任何一條更正被推翻。** 唯一的分歧裁決是一條「方向對、範圍講太滿」的主張，不是一條
假話。

證據是具體的——精確行號、`grep -n` 輸出、實際執行 gate 得到的 stdout——而且註記真的在替落敗那方辯
護。其中一條還浮出一個沒人記錄過的缺陷:真實的 `git_gate.py` receipt 帶 22 個 gate，而
`check_plan_package_compat.py` 用一份 23 條的清單去測 `expected_gate_count`，**所以一份真 receipt
會讓那道檢查失敗**。

### 兩個必須謹慎讀的理由

**盲判只是部分成立。** 每一對裡「更正版」那句**系統性地更詳細**，因為它是在讀完源碼之後寫的。裁決
者**單憑詳細程度就可能推出哪句是哪句**——而那個啟發式又與「哪句是對的」相關，所以它**無法與裁決結
果分離**。每位裁決者確實各自產出了源碼證據，這減輕但沒有消除這個洩漏。

**22 條零棄權、全部 high confidence**，儘管指令明確要求源碼沒說就棄權。**這既可能是樣本本來就明
確，也可能是儀器不具鑑別力。** 引用的證據支持前者，但 22 比 0 的全勝仍應照它的本來面目讀。

**n = 22 / 53。** 站得住的說法是:**多數更正成立，且抽樣中沒有一條被推翻**——不是「53 條全對」。
完整紀錄與抽樣規則在 [`data/correction-adjudication.json`](data/correction-adjudication.json)。

---

## Arm D,以及一個原本算錯的分母

arm D 就是 arm C 的程序加上閘迴圈——冷啟動、靶的準備方式完全相同、閘跑了 24 次。

| 以 `--exclude nonofficial` 量測 | 頁 | anchor | 無效 | 錨定率 | 覆蓋 | 成本 |
|---|---|---|---|---|---|---|
| A baseline | 31 | 0 | 0 | 0.0% | 93.8% | 未紀錄 |
| B retrofit | 31 | 312 | 0 | **100%** | 100% | ~2.88M tokens |
| Bs 剝離 | 31 | 0 | 0 | 0.0% | 96.9% | — |
| C 從頭寫、無閘 | 35 | 590 | 2 | 41.3% | 100% | 563K / 74 分 |
| **D 從頭寫 + 閘** | 37 | **1053** | **0** | **100%** | **100%** | **590K / 79 分** |

上表除成本外每一欄，都由 `harness/src/audit_arms.ts` 在每次 push 對
[`repo-snapshot/`](repo-snapshot/) 重算，並與 `harness/arms-baseline.json` 比對。
2026-08-05 之前這些是讀者只能選擇相信的數字：靶不在 repo 裡，而聲稱在審計五臂的那個 CI 步驟
只檢查了五個目錄存在。

**多 5% 的成本，換 41% 到 100% 的差距**，而且 arm D **比 arm C 寫得更多**（50 頁 vs 48、
36,730 字 vs 35,530）——**前科警告的縮範圍現象，即使在閘的壓力下也沒有出現**，而那正是前科所講的
條件。

它同時更正了本文稍早的一個說法:retrofit 的 2.88M 貴，是因為它用了二十幾個併發 agent 各自重讀源
碼，**不是因為查證本身昂貴**。閘迴圈是同一個 session 內的確定性檢查，幾乎免費。

### 閘抓到的是作者自己看不見的東西

arm D 自己的 notes:引文內的 `)` 讓 anchor 提早截斷、它沒意識到 heading 與表格列也算 claim、
以及**六條引文在源碼裡根本不逐字存在**——源碼折行、TypeScript 物件鍵沒有引號、`:` 實際上是 `?`。
它對這些的評語是「每一個都是真缺陷」。**一個只數「寫了幾個 anchor」的 checker，會把這些全部靜默
放過。**

### 分母原本是錯的,這是更正

arm B 那輪的指示**沒有禁止**修改 repo 自有的手寫 `nonofficial/` 頁，所以它錨了那些頁:
**486 個 anchor 裡有 174 個在那裡。** arm C 與 D 被要求保留那些頁，因此是 0。
**三臂、三個分母**，而本文稍早發佈的數字混了它們。

上表每個比率都以 `--exclude nonofficial` 計算。那 14 頁是**手寫的 repo 文檔，不是受測 pipeline 的
產物**，拿它們進分母去評 pipeline 是範疇錯誤;而它們也不能刪——五支腳本硬性要求（`check_openwiki.py`
就有 28 處），且讀者實際拿得到。**被量測的集合 ≠ 被交付的集合。**

更正後:arm B 是 **312** 個 anchor 不是 486;arm D 是 **100%** 不是它全域顯示的 59.8%——它那 80 條
未錨 claim **全在被禁止修改的頁裡**。**方向未變，量值原本是錯的。**

排除做成**閘的旗標而非慣例**，因為這裡失敗的正是慣例。

### arm D 沒有 QA 數字

public 30 題已用在四臂上。要給 arm D 一個可比數字得**重跑五臂**，而那沒有做。所以這裡**沒有任何東
西**說 arm D 比 arm C 更幫得上讀者——只說它錨得更完整、覆蓋相同、篇幅略大，代價多 5%。
