...
(src: scripts/git_gate.py `"""Run local regression gates for skill asset changes."""`).
`.githooks/pre-push` is four lines and calls it and nothing else
(src: .githooks/pre-push `python3 "$ROOT/scripts/git_gate.py"`), and
`.github/workflows/skill_ci.yml` adds only checkout and Python setup before the same call
(src: .github/workflows/skill_ci.yml `run: python scripts/git_gate.py`).
...
list (src: scripts/git_gate.py `is deliberately NOT gated here`), because the chain passes no
arguments and that path is only a schema check
(src: scripts/git_gate.py `schema check that never walks history at all`). Lineage is validated
where the commits actually live, by an explicit out-of-chain run
(src: scripts/git_gate.py `python3 scripts/validate_molecular_commit_lineage.py`).
...
(src: scripts/git_gate.py `exit_code = 125`) and prints
`FAIL: git gate changed receipt-bound repo inputs`
(src: scripts/git_gate.py `FAIL: git gate changed receipt-bound repo inputs`), regardless of
...
(src: scripts/git_gate.py `[sys.executable, str(root / gate)],`), and without `--write` that
renderer only compares its rendering against the checked-in file
(src: scripts/render_lifecycle_openwiki.py `if observed != output:`). Writing is an author
...
(src: scripts/validate_commit_message.py `if not argv or argv == ["--selftest"]:`) — or
