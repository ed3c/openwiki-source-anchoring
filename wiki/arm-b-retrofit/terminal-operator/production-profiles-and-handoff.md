...
|---|---|---|
| `production-journey.ts` | `repo-terminal-production-journey-receipt@v1` | **No — legacy only** (src: .agents/skills/repo-terminal-operator/SKILL.md `is legacy-only and MUST NOT be used for admission`) |
| `writer-production-journey.ts` (local) | `repo-terminal-writer-production-journey-receipt@v1` (src: .agents/skills/repo-terminal-operator/writer-production-journey.ts `schema_version: "repo-terminal-writer-production-journey-receipt@v1",`) | **No** — the gate demands the `@v2` schema (src: .agents/skills/repo-terminal-operator/small-loop-gate-contract.ts `"repo-terminal-writer-production-journey-receipt@v2";`) |
| `os-contained-production-profile.ts` | preflight receipt | **No** — it (src: .agents/skills/repo-terminal-operator/SKILL.md `remains available for diagnosis but cannot admit writer safety`) |
| `writer-contained-production-profile.ts` (upstream, named as the only production-use command in `production-use.profile.json` (src: .agents/skills/repo-terminal-operator/production-use.profile.json `skills/repo-neural-perception/scripts/writer-contained-production-profile.ts`)) | v2 receipt | **Yes** — it invokes the real repo-local `writer-entrypoint.ts` (src: .agents/skills/repo-terminal-operator/SKILL.md `inside the OS-contained carrier`) |
...
(src: .agents/skills/repo-terminal-operator/bounded-subprocess.ts `runBoundedProcess(command: string[], options: BoundedProcessOptions): Promise<BoundedProcessResult>`),
...
rejected as one (src: .agents/skills/repo-terminal-operator/forgejo-git-handoff-cli.ts `request schema is not an admitted Git handoff contract`). Two request types:
