...
`.githooks/pre-push` → `scripts/git_gate.py`
...
, which is why a documentation change can fail a
push. `scripts/check_wiki_graph_sync.py`
...
, runs only `scripts/ablation_engine.py`.
...
quoted sentence above is reproduced solely because `check_openwiki.py` matches by substring containment
 and still expects it; nothing on this page
