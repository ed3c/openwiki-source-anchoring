...
上面的一切驗證的都是 retrofit。skill 早已被改成在生成步驟引用附錄，而**那條路徑一次都沒被執行
過**——所以被證明的是 harness，不是 skill。

arm C 就是那次執行：改過的 skill、冷跑、在靶的乾淨複本上，移除了 arm A 的 review 逐字稿與生成
頁面，而且提示詞**不含**上述任何一項發現。

它把整個 workflow 跑完了。它寫了 **590 個 anchor——比 retrofit 的 486 更多——而錨定率只有
27.2%**，剔除它繼承且無權編輯的頁面後是 41.3%。**retrofit 是 100%。**

```mermaid
flowchart LR
    CONV["慣例<br/><i>什麼是 anchor</i>"] --> P41["≈41%"]
    CONV --> GATE["+ 閘<br/><i>哪些 claim 還沒有</i>"]
    GATE --> P100["100%"]
    ARMC["arm C<br/>只有慣例"] -.-> P41
    ARMB["arm B<br/>慣例 + 閘驅動迴圈"] -.-> P100
...
原因寫在 arm C 自己的 notes 裡:它從未跑過那道閘，而它自建的 checker 數的是**寫了**幾個 anchor，
不是**還有**幾條 claim 沒錨。**這是兩個不同的問題。**

> **慣例告訴作者「好長什麼樣」；只有閘會告訴作者「你還差在哪」。**

# 第五件沒有預料到的事

## 5 · 一直把慣例當成機制本身

上面每一個階段都把錨形當成介入手段、把閘當成它的計分員。**arm C 顯示這是反的**:形式值約 41%，
而**那個會回報「還缺什麼」的迴圈值其餘的部分**。把附錄接進生成提示詞就宣告完成，出貨的**不到量測
到的一半**。

推論令人不安且具一般性:**arm B 報出來的數字是「閘驅動迴圈」的性質，不是「錨定慣例」的性質。**
任何只採用慣例的人應該預期約 41%，而**若不是 arm C 跑了這一次，這個 repo 會暗示相反的事**。

另外兩個確證是免費附送的。arm C 的兩個無效 anchor 都指向 wiki 自己的產物——附錄用文字禁止了它、
arm C 自己的 checker 裡沒有這條、於是它自報乾淨；**規則寫在提示詞、缺在工具裡，最後算數的是工具**。
以及 arm C 在從未看過的情況下**獨立推導出畸形 anchor 的偵測規則**，這使它成為**錨形的性質**，
而不是一個曾經被修好的 bug。
