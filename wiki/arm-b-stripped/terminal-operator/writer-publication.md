...
| Module | Publishes |
|---|---|
| `writer-publication.ts` | owner — it declares the function itself |
| `writer-entrypoint.ts` | the public writer seam, and the admission-relevant one; it emits the writer receipt |
...
| `evidence-cost-collector.ts` | request, execution, observation and completion receipts, all through one content-addressed helper |
