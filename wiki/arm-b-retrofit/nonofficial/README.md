...
- **This page** is the human navigation index. It is *not* the declared `openwiki_entry` — that gate pins
  the entry to quickstart instead (src: scripts/check_openwiki.py `openwiki_entry: openwiki/quickstart.md`),
  and the key lives in `openwiki/nonofficial/openwiki.yaml`, one level deeper than `openwiki/openwiki.yaml`.
  What `scripts/check_openwiki.py` demands of *this* file is that it exist and still contain the eight
  literal section names it greps for.
...
- **[Skill asset contract](../skill-assets/contract.md)** — the `skills.md` / `cases.json` / `references/`
  shape. `scripts/validator.py` enforces only the `cases.json` half of that shape
  (src: scripts/validator.py `Validate skill cases.json files with the GCR minimum baseline.`);
  `skills.md` structure is a separate gate, `scripts/validate_progressive_disclosure.py`.
...
- **[Structured Lifecycle Data](structured-lifecycle-data.md)** — generated display layer for
  `data/lifecycle/`. **Never hand-edit**: `scripts/check_lifecycle_datasets.py` compares the page text
  against `scripts/render_lifecycle_openwiki.py --stdout` and fails on any difference
...
- **[Defense gate chain](../architecture/defense-gate-chain.md)** — `scripts/git_gate.py` internals and its
  input-state guarantee: it hashes every non-excluded file before and after the run and reports whether the
  two digests match (src: scripts/git_gate.py `def input_state_sha256(root: Path) -> str:`).
