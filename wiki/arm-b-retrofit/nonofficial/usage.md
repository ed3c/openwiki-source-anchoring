...
Requires Python ≥ 3.11 (src: pyproject.toml `requires-python = ">=3.11"`). All 17 lines of
`pyproject.toml` declare no dependency table at all, so there is no third-party runtime dependency;
...
python3 scripts/git_gate.py              # the whole 22-gate chain
```

`git_gate.py` is also what `.githooks/pre-push`
(src: .githooks/pre-push `python3 "$ROOT/scripts/git_gate.py"`) and `.github/workflows/skill_ci.yml`
(src: .github/workflows/skill_ci.yml `run: python scripts/git_gate.py`) run, so a green
...
Note the argument in the last row. Under `git_gate.py` every gate is launched with **no arguments**
(src: scripts/git_gate.py `[sys.executable, str(root / gate)],`), so `validate_commit_message.py`
(src: scripts/validate_commit_message.py `if not argv or argv == ["--selftest"]:`) and
`validate_goal_constraints.py` (src: scripts/validate_goal_constraints.py `if not args.paths:`) both
take their selftest branch instead of validating anything real — see
...
| `python3 scripts/check_plan_package_compat.py` | `PROJECT-SSOT.md`, `plan-package.compat.yaml`, or `.plan-package.lock.yaml` changed (src: scripts/check_plan_package_compat.py `manifest = parse_manifest(ROOT / "plan-package.compat.yaml")`) |
...
(src: scripts/git_gate.py `parser.add_argument("--receipt", type=Path)`):
