...
1. **Skill assets** — `skills/<slug>/{skills.md,cases.json,references/}`. Two slugs today, and they
   ...
   `status.json` (src: skills/gemini_interactions/status.json `"status": "quarantined"`), while
...
2. **Deterministic defense gates** — 30 Python scripts under `scripts/`, 22 of them sequenced by
   `scripts/git_gate.py` (src: scripts/git_gate.py `for gate in GATES:`). That 22 is the length of the
   ...
   (src: scripts/llm_judge.py `Local parser for LLM judge verdicts`). See
...
(src: scripts/git_gate.py `"scripts/check_openwiki.py",`), a stale wiki fails a push. The graph is a
...
(src: plan-package.compat.yaml `final_repo_forbidden_paths: small-loop,packets,templates/skill-defense-governance`). See
...
(src: scripts/real_driver_ablation.py `parser.add_mutually_exclusive_group(required=True)`). Read
