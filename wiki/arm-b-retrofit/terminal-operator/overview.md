...
(src: .agents/skills/repo-terminal-operator/generated-core/manifest.json `repo-neural-perception-derived-core@v1`)
...
(src: .agents/skills/repo-terminal-operator/task-quality-contract.ts `"../../../../tests/skills/repo-terminal-task-quality.test.ts",`).
...
(src: .agents/skills/repo-terminal-operator/repo-preflight.ts `resolve(workspaceRoot, "runtime/contracts/validate-packet.ts")`),
...
(src: .agents/skills/repo-terminal-operator/repo-adapter.ts `const outputRepo = resolve(root, "../../..");`),
which lands on the repository root, and `workspaceRoot` two above that
(src: .agents/skills/repo-terminal-operator/repo-adapter.ts `const workspaceRoot = resolve(outputRepo, "../..");`):
...
(src: .agents/skills/repo-terminal-operator/task-quality-contract.ts `export const TASK_QUALITY_STAGE_DEFINITIONS`);
`task-quality-runner.ts::executeTaskQuality()` runs them
(src: .agents/skills/repo-terminal-operator/task-quality-runner.ts `export async function executeTaskQuality(`).
