...
## `ablation_engine.py` — simulated, deterministic, in the default chain (src: scripts/git_gate.py `"scripts/ablation_engine.py",`)
...
| Dataset | Telemetry |
|---|---|
| `skills/gemini_interactions/cases.json` (default, per the argparse default (src: scripts/ablation_engine.py `default=ROOT / "skills" / "gemini_interactions" / "cases.json"`)) | `delta=0.50 case_count=10 success_rate=1.0 verdict=PASS` |
| `skills/autoresearch_composer/cases.json` (the path the lifecycle checker re-runs (src: scripts/check_autoresearch_lifecycle.py `REPO_CASES = ROOT / "skills" / "autoresearch_composer" / "cases.json"`)) | `delta=1.0 case_count=12 with_skill=1.0 without_skill=0.0`, `positive_case_count=6`, `negative_case_count=6`, `negative_with_skill_success_rate=1.0` |

`scripts/check_autoresearch_lifecycle.py` re-runs that second row and rejects the result unless five
conditions all hold (src: scripts/check_autoresearch_lifecycle.py `telemetry.get("negative_with_skill_success_rate") != 1.0`):
...
## `real_driver_ablation.py` — real agent (src: scripts/real_driver_ablation.py `Run a real agent command against the same cases with and without skill context.`), deliberately outside every automatic trigger
...
`tests/test_real_driver_ablation.py` (src: tests/test_real_driver_ablation.py `def test_nonzero_agent_exit_can_never_produce_a_passing_ablation(self) -> None:`):
...
## `benchmark_runner.py` — a fixed matrix, not a measurement (src: scripts/benchmark_runner.py `Deterministic benchmark matrix for skill quality deltas.`)
...
## `github_skill_harvester.py` — local inventory only (src: scripts/github_skill_harvester.py `Harvest local skill asset metrics without network access.`)
...
|---|---|
| "the case corpus still behaves as specified" | `ablation_engine.py` (src: scripts/ablation_engine.py `Deterministic local ablation runner for skill assets with regex telemetry.`) |
| "the scoring code did not regress" | `benchmark_runner.py`, whose fixed numbers are asserted in the governance test (src: tests/test_skill_asset_governance.py `assert "delta_high_quality=0.15" in benchmark.stdout`) |
| "this skill improves a real agent" | **only** `real_driver_ablation.py`, with a preregistered threshold (src: scripts/real_driver_ablation.py `delta >= args.threshold`) |
