...
`writer-publication.ts` owns `publishWriterArtifact()`
(src: .agents/skills/repo-terminal-operator/writer-publication.ts `export function publishWriterArtifact(`),
the one function every other module uses to put a byte on disk durably. A change here affects everything.

## Nine consumers

| Module | Publishes |
|---|---|
| `writer-publication.ts` | owner — it declares the function itself (src: .agents/skills/repo-terminal-operator/writer-publication.ts `export function publishWriterArtifact(`) |
| `writer-entrypoint.ts` | the public writer seam, and the admission-relevant one; it emits the writer receipt (src: .agents/skills/repo-terminal-operator/writer-entrypoint.ts `schema_version: "repo-terminal-writer-receipt@v1",`) |
| `async-worker-carrier.ts` | worker transaction state and results (src: .agents/skills/repo-terminal-operator/async-worker-carrier.ts `function prepareResultTransaction(`) |
| `async-job-lifecycle.ts` | seals and CAS lifecycle events (src: .agents/skills/repo-terminal-operator/async-job-lifecycle.ts `export function sealAsyncProductionRun(`) |
| `async-admission-facade.ts` | the immutable ownership receipt (src: .agents/skills/repo-terminal-operator/async-admission-facade.ts `"repo-async-production-admission@v1",`) |
| `async-control-plane.ts` | projected queue and redrive refs (src: .agents/skills/repo-terminal-operator/async-control-plane.ts `ASYNC-PRODUCTION-REDRIVE-PROGRESS-PROJECTOR`) |
| `async-progress-store.ts` | worker progress (src: .agents/skills/repo-terminal-operator/async-progress-store.ts `export function persistAsyncWorkerProgress(`) |
| `evidence-cost-cache.ts` | canonical requests and cache entries (src: .agents/skills/repo-terminal-operator/evidence-cost-cache.ts `export function canonicalEvidenceCostRequestBytes(`) |
| `evidence-cost-collector.ts` | request, execution, observation and completion receipts, all through one content-addressed helper (src: .agents/skills/repo-terminal-operator/evidence-cost-collector.ts `function publishContentAddressed(`) |
...
toolchain and no `package.json` anywhere in the tree (inferred; a repo-wide search for that name
returns nothing), and the admission-valid contained profile is reached by a relative path that climbs
out of the only vendored skill directory into a `repo-neural-perception` tree that is absent here
(src: .agents/skills/repo-terminal-operator/SKILL.md `skills/repo-neural-perception/scripts/writer-contained-production-profile.ts`).
See [Terminal operator overview](overview.md) and
