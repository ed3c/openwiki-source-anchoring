...
`pyproject.toml` sets `testpaths = ["tests"]` (src: pyproject.toml `testpaths = ["tests"]`) and
registers three markers: `evals` (local-first golden dataset and deterministic judge), `llm_judge`
(src: pyproject.toml `cloud path disabled by default`), `trace` (local trace sampling and guardrails).
...
## `tests/test_skill_asset_governance.py` (src: tests/test_skill_asset_governance.py `def test_skill_asset_structure() -> None:`)
...
(src: tests/test_skill_asset_governance.py `Asserts the selftest, not the ledger.`), because the
...
## `tests/test_autoresearch_eval_suite.py` (src: tests/test_autoresearch_eval_suite.py `pytestmark = pytest.mark.evals`)
...
## `tests/test_real_driver_ablation.py` (src: tests/test_real_driver_ablation.py `class RealDriverAblationTest(unittest.TestCase):`)
...
(src: tests/fixtures/real_driver_mock_agent.py `Codex-JSONL shaped fixture for the real-driver ablation public seam.`) — so
...
expects a Bun toolchain (src: .agents/skills/repo-terminal-operator/SKILL.md `bun run repo-adapter.ts --describe`),
