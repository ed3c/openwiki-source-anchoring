...
environment (src: scripts/sync_wiki_to_graph.py `if os.environ.get("ENABLE_GRAPH_DB_WRITE") != "true":`),
and it is only reached when the CLI was given `--write-external-graph`
(src: scripts/sync_wiki_to_graph.py `if args.write_external_graph:`). **Both** are
