...
> (src: skills/gemini_interactions/status.json `"status": "quarantined"`),
...
model `gpt-5.6-sol`
(src: data/verification_runs/gemini_interactions_real_driver_2026-07-27.json `"model": "gpt-5.6-sol"`).
...
`status.json` records four reasons (src: skills/gemini_interactions/status.json `"reasons": [`), and
only the first is about the number:
...
`scripts/synthetic_case_generator.py` produces 117 fixtures, typescript=59 and python=58
...
zero_llm_api_calls=0`, a target it hard-asserts
(src: scripts/interactions_patch_assert_runner.py `!= 117 or telemetry["zero_llm_api_calls"] != 0`);
`scripts/local_regex_runner.py` reports `case_count=10 total_trials=50`, ten cases at five trials each
(src: scripts/local_regex_runner.py `TRIALS_COUNT = 5`); `scripts/ablation_engine.py` on the default
gemini cases reports `delta=0.50` against a target of 0.05
(src: scripts/ablation_engine.py `TARGET_DELTA = 0.05`).
...
(src: tests/test_real_driver_ablation.py `def test_each_agent_call_uses_an_ephemeral_cwd`):
...
| per-invocation ephemeral cwd | each invocation runs in its own directory, so agent side effects cannot leak between samples | `test_each_agent_call_uses_an_ephemeral_cwd` |
...
**No gate reads `status.json`.** `scripts/git_gate.py::GATES` is a flat list of runnable gate scripts
(src: scripts/git_gate.py `GATES = [`) that ends at `scripts/no_ops_purger.py` with no status check in
it; the only place the status file is bound to its receipt is a field inside the status file itself
...
and grepping `scripts/`, `tests/`, `.github/workflows/` and `.githooks/` for that field name returns no
hit outside it. Nothing prevents this asset from being loaded; the file records a decision a human made.
...
3. Run `scripts/real_driver_ablation.py` with the ephemeral per-invocation cwd and
   ...
   with complete runtime metadata and zero timeouts.
