...
. The native-yield routes are the
...
demands those four tokens
and additionally caps the file at 200 words;
`scripts/validate_progressive_disclosure.py` demands the same signals plus an actual `references/`
directory next to the skill;
...
,
no weak check patterns,
at least five positive and five negative cases
, and no two lowercased prompts above a
0.85 `SequenceMatcher` similarity ratio. See
...
the first failure. The mirrored `GIT_GATE_ORDER` in
...
and carry their required literals — this page
